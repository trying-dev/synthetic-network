...
mod selftest {
    ...
    fn checksum() {
        let cases: Vec<&[u8]> = vec![
            ...
            &[],
            &[
                01u8, 02, 03, 04, 05, 06, 07, 08, 09, 10, 11, 12, 13, 14, 15, 16, 01u8, 02, 03, 04,
                05, 06, 07, 08, 09, 10, 11, 12, 13, 14, 15, 16, 01u8, 02, 03, 04, 05, 06, 07, 08,
                09, 10, 11, 12, 13, 14, 15, 16, 01u8, 02, 03, 04, 05, 06, 07, 08, 09, 10, 11, 12,
                13, 14, 15,
            ],
        ];
        for case in cases {
    ...
    fn checksum_random() {
        let mut progress = 1;
        for i in 1..=32 {
            // Crank this up to run more random test cases
            if i >= progress {
                println!("{}", progress);
                progress *= 2;
            }
            for l in 0..=1500 {
                // Tune this down (to e.g. 63) for faster cases
                let mut case = vec![0u8; l];
    ...
    fn checksum_rampool_bench() {
        ...
        println!(
            "Checksummed {} * {} byte chunks (RAM pool size {})",
